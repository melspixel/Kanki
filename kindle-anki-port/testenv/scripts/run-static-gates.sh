...
run python3 -m py_compile \
    ...
    "$ROOT/tests/test_core_integration.py" "$ROOT/tests/test_apkg_prepare.py" \
...
run python3 "$ROOT/tests/test_apkg_prepare.py"
...
run python3 "$ROOT/tests/test_host_backend_provenance.py"
run python3 "$ROOT/tests/test_qemu_provenance.py"
...
run node --check "$ROOT/web/ime.js"
run node --check "$ROOT/web/css_compat.js"
run node --check "$ROOT/web/audio_queue.js"
run node --check "$ROOT/web/decks.js"
run node --check "$ROOT/web/reviewer.js"
run node "$ROOT/tests/test_audio_queue.js"
run node "$ROOT/tests/test_ime_runtime.js"

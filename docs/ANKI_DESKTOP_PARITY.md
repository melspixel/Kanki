...
| TTS metadata/runtime | Preserve text, language, preferred voices and speed; select an available host voice | Direct and ordered protocols preserve all fields; fixed-PW6 adapter sets `textsource`, `voicelang` and clamped native `speed` on `ttssrc` before `mixersink` | Executable browser/parser/fake-runtime contracts plus actual fixed-rootfs property inspection and packaged pipeline construction pass; PW6 exposes no voice-ID property, and audible language/routing evidence remains open |
